...
void FastAnim_Decompress(struct Model *m, int j, int k)
{
	...
	for (int i = 0; i < numFrames; i++)
	{
		...
		// AlignUp
		unsigned int byte = (unsigned int)ptrMempack->firstFreeByte;
		byte += 3;
		byte &= ~(3);
...
void FastAnim_GenerateLerp(struct Model *m, int j, int k)
{
	...
	for (int i = 0; i < (numFramesPreLerp - 1); i++)
	{
		// old frame
		{
			...
			struct ModelFrame *newMF = &newFirstFrame[newMA->frameSize * ((i * 2) + 0)];
			...
			printf("old %08x new %08x\n", MODELFRAME_GETVERT(mf), MODELFRAME_GETVERT(newMF));
		}
		...
		// new frame
		{
			...
			struct ModelFrame *newMF = &newFirstFrame[newMA->frameSize * ((i * 2) + 1)];
