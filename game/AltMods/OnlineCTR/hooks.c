...
void OnlineInit_Drivers(struct GameTracker *gGT)
{
	...
	if ((gGT->levelID > TURBO_TRACK) && (gGT->levelID < GEM_STONE_VALLEY))
	{
		// 4 spawn positions
		bitFlag = 3;
	}
...
void OnlineEndOfRace()
{
	...
	if (((driver->actionsFlagSet & 0x2000000) == 0) || (octr->CurrState < GAME_START_RACE))
	{
		return;
	}
...
void Online_OtherFX_RecycleNew(u_int *soundID_Count, u_int newSoundID, u_int modifyFlags)
{
	struct Driver *d = sdata->gGT->drivers[0];

	if (((int)soundID_Count != (int)&d->driverAudioPtrs[0]) && ((int)soundID_Count != (int)&d->driverAudioPtrs[1]) &&
	    ((int)soundID_Count != (int)&d->driverAudioPtrs[2]) && ((int)soundID_Count != (int)&d->driverAudioPtrs[3]))
	{
